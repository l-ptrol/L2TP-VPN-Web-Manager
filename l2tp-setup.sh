...
# Читаем порт из первого аргумента (по умолчанию 8081)
PORT=$1
if [ -z "$PORT" ]; then
    PORT=8081
fi
...
if [ ! -f "$CONFIG_FILE" ]; then
    echo "[+] Создание конфигурационного файла по умолчанию в $CONFIG_FILE с портом $PORT..."
    cat <<EOF > "$CONFIG_FILE"
{
  "web_port": $PORT,
  "profiles": [],
...
EOF
    ...
    echo "[+] Конфигурационный файл уже существует. Обновление порта веб-панели на $PORT..."
...
echo "=================================================="
echo "Веб-интерфейс будет доступен по адресу: http://${LOCAL_IP}:${PORT}"
